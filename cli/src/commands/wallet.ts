...
import { getNativeBalance, getPublicClient, formatEther } from '../lib/client.js';
...
walletCommand
  ...
  .action(async () => {
    try {
      ...
      const method = await promptSelect<'generate' | 'privateKey' | 'mnemonic'>(
...
walletCommand
  ...
  .action(async () => {
    try {
      ...
      if (!address) {
        printError('No wallet configured', 'Run "pickle wallet setup" to create one');
        process.exit(1);
      }
...
walletCommand
  ...
  .action(async () => {
    try {
      ...
      printKeyValue('XLM', styled.primary(formatEther(nativeBalance)));
