...
tradeCommand
  ...
  .description('Buy tokens with XLM')
    ...
    try {
      ...
      if (balance < amountWei) {
        ...
        printError(
          ...
          `You have ${formatEther(balance)} XLM but need ${amount} XLM`
        );
        process.exit(1);
      }
      ...
      printKeyValue('You pay', styled.warning(`${amount} XLM`));
      ...
      try {
        ...
        newLine();
        printSuccess(`Bought ${formatTokenAmount(tokensReceived)} ${tokenInfo.symbol} for ${amount} XLM`);
        printTxLink(hash);
      } catch (error) {
...
tradeCommand
  ...
  .description('Sell tokens for XLM')
  .option('-s, --slippage <percent>', 'Slippage tolerance in percent', '1')
  .action(async (token, amount, options) => {
    try {
      // Check wallet
      ...
      if (tokenBalance < amountWei) {
        ...
        printError(
          ...
          `You have ${formatTokenAmount(tokenBalance)} but want to sell ${amount}`
        );
        process.exit(1);
      }
      ...
      try {
        ...
        newLine();
        printSuccess(`Sold ${amount} ${tokenInfo.symbol} for ${formatEther(ethReceived)} XLM`);
        printTxLink(hash);
      } catch (error) {
        txSpinner.fail('Transaction failed');
        printError((error as Error).message);
  ...
  .action(async (token, amount, options) => {
    try {
      ...
      if (options.type === 'buy') {
        ...
        newLine();
        printKeyValue('Input', `${amount} XLM`);
