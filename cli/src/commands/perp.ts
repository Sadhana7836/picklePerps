...
async function resolveToken(tokenInput: string): Promise<string | null> {
...
perpCommand
  ...
  .action(async (positionIdStr) => {
    try {
      ...
      printKeyValue('Size', `${position.size} XLM`);
      printKeyValue('Margin', `${position.margin} XLM`);
      printKeyValue('Entry Price', formatPrice(parseFloat(position.entryPrice)));
      printKeyValue('PnL', styled.pnl(formatEther(pnl) + ' XLM', isProfit));
      newLine();
      ...
      try {
        ...
        printKeyValue('Realized PnL', styled.pnl(formatEther(pnl) + ' XLM', isProfit));
        printTxLink(hash);
      } catch (error) {
    ...
    try {
      ...
      for (const pos of positions) {
        ...
        const pnlStr = pos.pnl
          ? styled.pnl(pos.pnl + ' XLM', pos.pnlIsProfit || false)
          : styled.muted('--');
  ...
  .action(async (positionIdStr) => {
    try {
      ...
      if (positionIdStr) {
        ...
        newLine();
        printKeyValue('Unrealized PnL', styled.pnl(formatEther(pnl) + ' XLM', isProfit));
        ...
        for (const pos of positions) {
          ...
          console.log(
            `  #${pos.positionId} ${tokenSymbol} ${pos.isLong ? styled.long('LONG') : styled.short('SHORT')}: ` +
            styled.pnl(formatEther(pnl) + ' XLM', isProfit)
          );
        }

        newLine();
        const totalIsProfit = totalPnl >= 0n;
        printKeyValue('Total PnL', styled.pnl(formatEther(totalIsProfit ? totalPnl : -totalPnl) + ' XLM', totalIsProfit));
      }
    } catch (error) {
      printError((error as Error).message);
      process.exit(1);
