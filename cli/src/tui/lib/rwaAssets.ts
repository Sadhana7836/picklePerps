...
// Get asset by ID
export function getAssetById(id: string): RWAAsset | undefined {
  return RWA_ASSETS.find(a => a.id.toLowerCase() === id.toLowerCase());
}

// Get assets by category
export function getAssetsByCategory(category: RWAAsset['category']): RWAAsset[] {
  return RWA_ASSETS.filter(a => a.category === category);
}

// Category display names
export const CATEGORY_NAMES: Record<RWAAsset['category'], string> = {
