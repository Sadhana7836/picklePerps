...
import { getBuyQuote, getSellQuote, buyTokens, sellTokens, openPosition, closePosition } from '../lib/contracts.js';
import { formatEther } from '../lib/client.js';
...
// Buy token dialog
export async function showBuyDialog(
  screen: blessed.Widgets.Screen,
  token: Token,
  ...
  {bold}Fee:{/bold} ${fee.toFixed(6)} XLM
  ...
  {green-fg}[C]{/} Confirm Trade  {red-fg}[Escape]{/} Cancel
      ...
      dialog.key(['c', 'C'], async () => {
        ...
        try {
          ...
          loadingBox.destroy();
          screen.render();
          onComplete(false, `Trade failed: ${(error as Error).message}`);
        }
      });
    } catch (error) {
...
// Sell token dialog
export async function showSellDialog(
  screen: blessed.Widgets.Screen,
  token: Token,
  onComplete: (success: boolean, message: string) => void
): Promise<void> {
  ...
  input.on('submit', async (value) => {
    ...
    try {
      ...
      quoteBox.setContent(`
  {bold}You will receive:{/bold} {green-fg}${ethOut.toFixed(6)} XLM{/green-fg}
  {bold}Average Price:{/bold} $${avgPrice.toFixed(8)}
  {bold}Fee:{/bold} ${fee.toFixed(6)} XLM
  {bold}Price Impact:{/bold} ${quote.priceImpact.toFixed(2)}%
  ...
  {green-fg}[C]{/} Confirm Trade  {red-fg}[Escape]{/} Cancel
      ...
      dialog.key(['c', 'C'], async () => {
        ...
        try {
          ...
          loadingBox.destroy();
          screen.render();
          onComplete(true, `Successfully sold ${value} ${token.symbol} for ${(Number(result.ethReceived) / 1e18).toFixed(6)} XLM!\nTx: ${result.hash.slice(0, 20)}...`);
        } catch (error) {
          loadingBox.destroy();
          screen.render();
          onComplete(false, `Trade failed: ${(error as Error).message}`);
...
export async function showOpenPositionDialog(
  screen: blessed.Widgets.Screen,
  token: Token,
  isLong: boolean,
  onComplete: (success: boolean, message: string) => void
  ...
  {bold}Position Size:{/bold} ${size.toFixed(2)} XLM
  ...
  const submitPosition = async () => {
    ...
    try {
      ...
      loadingBox.destroy();
      screen.render();
      onComplete(false, `Failed: ${(error as Error).message}`);
    }
  };
...
// Wallet setup dialog
export async function showWalletSetupDialog(
  screen: blessed.Widgets.Screen,
  onComplete: (success: boolean, message: string) => void
): Promise<void> {
  ...
  dialog.key(['i', 'I'], async () => {
    ...
    keyInput.on('submit', async (value) => {
      try {
        ...
        keyDialog.setContent(`\n  {red-fg}Invalid private key: ${(error as Error).message}{/red-fg}`);
        screen.render();
      }
    });
  ...
  dialog.key(['m', 'M'], async () => {
    ...
    mnemonicInput.on('submit', async (value) => {
      try {
        ...
        mnemonicDialog.setContent(`\n  {red-fg}Invalid mnemonic: ${(error as Error).message}{/red-fg}`);
        screen.render();
      }
    });
...
async function promptNewPassword(screen: blessed.Widgets.Screen): Promise<string | null> {
  return new Promise((resolve) => {
