...
import { formatAddress, formatNumber, formatTimeAgo } from '../lib/theme.js';
import { getWalletAddress, isWalletConfigured, saveWallet, generateNewWallet, walletFromMnemonic, accountFromPrivateKey } from '../lib/wallet.js';
import { getNativeBalance, formatEther } from '../lib/client.js';
import { fetchTrendingTokens, searchTokens } from '../lib/subgraph.js';
import { getAllUserPositions, getBuyQuote, getSellQuote, buyTokens, sellTokens, openPosition, closePosition } from '../lib/contracts.js';
...
export class App {
    ...
    initializeUI() {
        ...
        this.header = blessed.box({
            ...
            height: 8,
            tags: true,
            content: HEADER_LOGO + '\n{center}{gray-fg}Trade Meme Tokens & RWA on Mantle{/gray-fg}{/center}',
            style: {
                fg: 'white',
            },
        });
 ...
 {green-fg}[R]{/green-fg} Refresh Balance    {red-fg}[Q]{/red-fg} Quit`;
    ...
    setupKeyBindings() {
        ...
        // Tab navigation
        this.screen.key(['tab'], () => {
            if (this.screen.focused === this.tokenList) {
                this.sidebar.focus();
            }
            else {
        ...
        this.tokenList.on('select', (item, index) => {
        ...
        this.tokenList.key(['up', 'down', 'k', 'j'], () => {
            const index = this.tokenList.selected || 0;
            if (this.state.currentView === 'positions') {
                if (this.state.positions[index]) {
                    this.updatePositionDetails(this.state.positions[index]);
                }
        ...
        this.sidebar.on('select', (item, index) => {
            const views = ['dashboard', 'trade', 'positions', 'portfolio', 'wallet'];
    ...
    updateTokenList() {
        const items = this.state.tokens.map((token, i) => {
            ...
            return ` ${token.symbol.padEnd(8)} ${priceStr.padEnd(14)} ${formatNumber(parseFloat(token.totalVolume) / 1e18)} MNT`;
        });
        if (items.length === 0) {
 ...
 {bold}Volume:{/bold}     ${volume.toFixed(4)} MNT
...
${token.twitter ? ` {bold}Twitter:{/bold} ${token.twitter}` : ''}
    ...
    updatePositionsList() {
        if (this.state.positions.length === 0) {
            ...
            const items = this.state.positions.map(pos => {
                ...
                return ` #${pos.positionId.toString().padEnd(4)} ${side.padEnd(20)} ${pos.size} MNT  ${pnl}`;
            });
            this.tokenList.setItems(items);
    ...
    updatePositionDetails(pos) {
        const side = pos.isLong ? '{green-fg}LONG{/green-fg}' : '{red-fg}SHORT{/red-fg}';
        const pnl = pos.pnl
            ? (pos.pnlIsProfit ? `{green-fg}+${pos.pnl} MNT{/green-fg}` : `{red-fg}${pos.pnl} MNT{/red-fg}`)
            : '{gray-fg}--{/gray-fg}';
        this.detailsPanel.setContent(`
 ...
 {bold}Side:{/bold}       ${side}
 {bold}Size:{/bold}       ${pos.size} MNT
 {bold}Margin:{/bold}     ${pos.margin} MNT
 ...
 You need a wallet to trade on Pike Perps.
     ...
     Use a 12/24 word recovery phrase
      `);
            ...
            this.tokenList.setItems([
                ...
                '',
                `Balance:`,
                `{green-fg}${this.state.walletBalance} MNT{/green-fg}`,
            ]);
            this.detailsPanel.setContent(`
 ...
 {green-fg}[R]{/green-fg} Refresh balance
    ...
    updatePortfolioView() {
        const items = [
            `{bold}MNT Balance:{/bold} {green-fg}${this.state.walletBalance} MNT{/green-fg}`,
            '',
        ...
        const pnlStr = totalPnl >= 0
            ? `{green-fg}+${totalPnl.toFixed(4)} MNT{/green-fg}`
            : `{red-fg}${totalPnl.toFixed(4)} MNT{/red-fg}`;
        this.detailsPanel.setContent(`
 ...
 ${pnlStr}
    ...
    updateStatusBar() {
        ...
        const wallet = this.state.walletAddress
            ? `{green-fg}${formatAddress(this.state.walletAddress)}{/green-fg} | {green-fg}${this.state.walletBalance} MNT{/green-fg}`
            : '{red-fg}No Wallet{/red-fg}';
        this.statusBar.setContent(` {bold}${viewName}{/bold} | ${wallet} | {green-fg}[1-5]{/green-fg} Nav  {green-fg}[B]{/green-fg}uy  {red-fg}[S]{/red-fg}ell  {green-fg}[L]{/green-fg}ong  {red-fg}[H]{/red-fg}ort  {green-fg}[R]{/green-fg}efresh  {red-fg}[Q]{/red-fg}uit`);
        this.screen.render();
    }
    // ============ TRADING DIALOGS ============
    async showBuyDialog() {
    ...
    async showTradeInputDialog(type, token) {
 ...
 {bold}Amount (${type === 'buy' ? 'MNT' : token.symbol}):{/bold}`,
        });
 ...
 {bold}Fee:{/bold} ${fee.toFixed(6)} MNT | {bold}Impact:{/bold} ${quote.priceImpact.toFixed(2)}%
 ...
 {green-fg}[Y]{/green-fg} Confirm  {red-fg}[N]{/red-fg} Cancel`);
                    ...
                    const fee = Number(quote.fee) / 1e18;
                    quoteBox.setContent(`
 {bold}You receive:{/bold} {green-fg}${ethOut.toFixed(6)} MNT{/green-fg}
 {bold}Fee:{/bold} ${fee.toFixed(6)} MNT | {bold}Impact:{/bold} ${quote.priceImpact.toFixed(2)}%
 ...
 {green-fg}[Y]{/green-fg} Confirm  {red-fg}[N]{/red-fg} Cancel`);
    ...
    async executeBuy(token, amount, minTokens) {
    ...
    async executeSell(token, amount, minEth) {
        const password = await this.promptPassword();
        if (!password)
            return;
        if (password.length < 6) {
        ...
        try {
            ...
            this.showMessage(`Success! Received ${ethFormatted} MNT`, 'success');
            // Refresh data to show updated balances
            await this.refreshData();
        }
        catch (error) {
    ...
    async showOpenPositionDialog(isLong) {
        if (!this.state.selectedToken) {
            this.showMessage('Select a token first', 'error');
            return;
        }
        ...
        blessed.text({ parent: dialog, top: 4, left: 2, content: 'Margin (MNT):', tags: true });
        ...
        const submit = async () => {
            ...
            const liqPrice = isLong
                ...
                : price * (1 + 0.9 / leverage);
            infoBox.setContent(`
 Size: ${size.toFixed(2)} MNT | Liq: $${liqPrice.toFixed(8)}
 {green-fg}[Y]{/green-fg} Confirm  {red-fg}[N]{/red-fg} Cancel`);
            dialog.key(['y', 'Y'], async () => {
                dialog.destroy();
                this.screen.render();
    ...
    async executeOpenPosition(token, isLong, margin, leverage) {
        const password = await this.promptPassword();
        if (!password)
            return;
        if (password.length < 6) {
    ...
    async generateWallet() {
        if (this.state.currentView !== 'wallet')
            return;
        if (isWalletConfigured()) {
 ...
 {red-fg}[Escape]{/red-fg} Cancel`);
    ...
    async importPrivateKey() {
        ...
        try {
            ...
            this.showMessage(`Invalid key: ${error.message}`, 'error');
        }
    }
    async importMnemonic() {
        ...
        try {
            ...
            this.showMessage(`Invalid mnemonic: ${error.message}`, 'error');
        }
    }
    // ============ SEARCH ============
