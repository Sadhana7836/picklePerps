...
const GOLDSKY_SUBGRAPH_URL = 'https://api.goldsky.com/api/public/project_cmj709d6q6eqo01w6advl8q19/subgraphs/pikeperps/2.1.0/gn';
const DEFAULT_SUBGRAPH_URL = process.env.PIKEPERPS_SUBGRAPH_URL || GOLDSKY_SUBGRAPH_URL;
// Generic fetch function for subgraph queries
async function fetchSubgraph(query, variables, url) {
...
export async function searchTokens(searchQuery) {
...
export async function fetchPositions(address, tokenAddress) {
    ...
    `;
    const variables = { userId: address.toLowerCase() };
    if (tokenAddress) {
        variables.tokenId = tokenAddress.toLowerCase();
    }
    const data = await fetchSubgraph(query, variables);
    if (!data?.positions) {
        return [];
    }
...
// Fetch recent trades
export async function fetchTrades(tokenAddress, limit = 50) {
    const query = tokenAddress
        ? `
...
// Fetch leaderboard
export async function fetchLeaderboard(period = 'all-time', limit = 20) {
    const query = `
  ...
  `;
    const data = await fetchSubgraph(query, { period, first: limit });
