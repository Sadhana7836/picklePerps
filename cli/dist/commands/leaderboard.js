...
import { printSection, printError, createTable, createSpinner, styled, formatNumber, formatAddress, newLine, } from '../lib/ui.js';
...
export const leaderboardCommand = new Command('leaderboard')
    ...
    try {
        ...
        try {
            ...
            printError('Subgraph not available', 'Set PIKEPERPS_SUBGRAPH_URL environment variable');
            return;
        }
        ...
        for (let i = 0; i < leaders.length; i++) {
            ...
            const pnlStr = pnl >= 0
                ? chalk.green(`+${formatNumber(pnl)} MNT`)
                : chalk.red(`${formatNumber(pnl)} MNT`);
            ...
            if (i === 0)
                ...
                rank = chalk.hex('#cd7f32')('🥉 3');
            table.push([
                rank,
                formatAddress(leader.user),
                pnlStr,
                formatNumber(parseFloat(leader.volume)) + ' MNT',
                leader.trades,
                `${leader.winRate}%`,
            ]);
        }
