...
const HoldingCard = memo(function HoldingCard({ holding }: { holding: TokenHolding }) {
  ...
  return (
    <div className="bg-[#111] rounded-lg p-4 border border-[#1a1a1a] hover:border-[#252525] transition-colors">
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-lg overflow-hidden bg-[#0d0d0d] border border-[#1a1a1a]">
            {holding.imageHash ? (
              <IPFSImage
          ...
          <div>
            ...
            <p className="text-[#555] text-xs">{holding.name}</p>
          </div>
        </div>
        <span className={`text-xs px-2 py-1 rounded ${holding.change24h >= 0 ? "bg-[#00d26a]/10 text-[#00d26a]" : "bg-[#ff4757]/10 text-[#ff4757]"}`}>
      ...
      <div className="space-y-2 text-sm">
        ...
        <div className="flex justify-between">
          ...
          <span className="text-[#00d26a] font-medium">${holding.value.toFixed(4)}</span>
        </div>
      </div>

      <div className="flex items-center justify-between mt-3 pt-3 border-t border-[#1a1a1a]">
        <button onClick={copyAddress} className="flex items-center gap-1 text-[#555] text-xs hover:text-white transition-colors">
          {copied ? <Check className="w-3 h-3" /> : <Copy className="w-3 h-3" />}
          {holding.address.slice(0, 6)}...{holding.address.slice(-4)}
        </button>
        <a href={`https://stellar.expert/explorer/testnet/contract/${holding.address}`} target="_blank" rel="noopener noreferrer" className="text-[#555] hover:text-white">
  ...
  return (
    <div className="bg-[#111] rounded-lg p-4 border border-[#1a1a1a] hover:border-[#252525] transition-colors">
          ...
          <div className="w-10 h-10 rounded-lg overflow-hidden bg-[#0d0d0d] border border-[#1a1a1a]">
            {position.tokenImageHash ? (
              <IPFSImage
                ...
                height={40}
                className="w-full h-full object-cover"
                fallback={<div className="w-full h-full flex items-center justify-center text-lg text-[#555]">T</div>}
              />
            ) : (
              <div className="w-full h-full flex items-center justify-center text-lg text-[#555]">T</div>
            )}
          </div>
          <div>
            <p className="text-white font-medium text-sm">{position.tokenSymbol}</p>
            <p className="text-[#555] text-xs">{position.tokenName}</p>
          </div>
        </div>
        <span className="text-[#444] text-xs">#{position.positionId}</span>
      </div>

      {/* Position Type & Leverage */}
      <div className="flex items-center gap-2 mb-3">
        <span className={`text-xs font-medium px-2 py-1 rounded ${position.isLong ? "bg-[#00d26a]/10 text-[#00d26a]" : "bg-[#ff4757]/10 text-[#ff4757]"}`}>
      ...
      <div className="flex justify-between items-center pt-3 border-t border-[#1a1a1a]">
        <div>
          <p className="text-[#555] text-xs">PnL</p>
          <p className={`text-sm font-medium ${position.isProfit ? "text-[#00d26a]" : "text-[#ff4757]"}`}>
            {position.isProfit ? "+" : "-"}${Math.abs(position.pnl).toFixed(4)}
          </p>
        </div>
        <div className="text-right">
          <p className="text-[#555] text-xs">Liq. Price</p>
          <p className="text-[#ff4757] text-xs">${position.liquidationPrice.toFixed(6)}</p>
        </div>
      </div>
      ...
      {position.token && (
        <Link
          ...
          className="flex items-center justify-center gap-1 mt-3 pt-3 border-t border-[#1a1a1a] text-[#00d26a] text-xs hover:text-[#00e676] transition-colors"
        >
...
const TransactionRow = memo(function TransactionRow({ tx }: { tx: Transaction }) {
  return (
    <div className="flex items-center justify-between py-3 border-b border-[#1a1a1a] last:border-0">
      <div className="flex items-center gap-3">
        <div className={`w-8 h-8 rounded-full flex items-center justify-center ${tx.type === "buy" ? "bg-[#00d26a]/10" : "bg-[#ff4757]/10"}`}>
          {tx.type === "buy" ? <TrendingUp className="w-4 h-4 text-[#00d26a]" /> : <TrendingDown className="w-4 h-4 text-[#ff4757]" />}
        </div>
        <div>
          <p className="text-white text-sm">{tx.type === "buy" ? "Bought" : "Sold"} <span className="font-medium">{tx.tokenSymbol}</span></p>
          <p className="text-[#555] text-xs">{new Date(tx.timestamp).toLocaleDateString()}</p>
        </div>
      </div>
      <div className="flex items-center gap-3">
        <div className="text-right">
          <p className="text-white text-sm">{parseFloat(tx.amount).toFixed(2)}</p>
          <p className="text-[#555] text-xs">{tx.value.toFixed(4)} XLM</p>
        </div>
        <a href={`https://stellar.expert/explorer/testnet/tx/${tx.txHash}`} target="_blank" rel="noopener noreferrer" className="text-[#555] hover:text-white">
          <ExternalLink className="w-3 h-3" />
        </a>
      </div>
    </div>
  ...
  return (
    <div className="bg-[#111] rounded-lg p-4 border border-[#1a1a1a]">
      ...
      <div className="space-y-2 text-xs mb-3">
        <div className="flex justify-between">
          <span className="text-[#555]">Leader</span>
          <span className="text-white font-mono">
            {position.leader.slice(0, 6)}...{position.leader.slice(-4)}
          </span>
        </div>
        <div className="flex justify-between">
          <span className="text-[#555]">Your Margin</span>
          <span className="text-white">{parseFloat(position.followerMargin).toFixed(4)} XLM</span>
        </div>
        <div className="flex justify-between">
          <span className="text-[#555]">Leader Margin</span>
          <span className="text-[#888]">{parseFloat(position.leaderMargin).toFixed(4)} XLM</span>
        </div>
      </div>

      <div className="flex justify-between items-center pt-3 border-t border-[#1a1a1a] text-xs">
        <span className="text-[#555]">Opened</span>
      ...
      {!isConnected ? (
        <div className="flex items-center justify-center h-[60vh]">
          <div className="text-center">
            <div className="w-14 h-14 rounded-full bg-[#111] border border-[#1a1a1a] flex items-center justify-center mx-auto mb-4">
              <Wallet className="w-6 h-6 text-[#555]" />
            </div>
        ...
        <div className="max-w-5xl mx-auto">
          {/* Subtitle */}
          <div className="mb-6">
            <p className="text-[#555] text-sm">Track your holdings and positions</p>
          </div>

          {/* Stats */}
          <div className="grid grid-cols-3 gap-4 mb-6">
            <div className="bg-[#111] border border-[#1a1a1a] rounded-lg p-4">
              <p className="text-[#555] text-xs mb-1">Total Value</p>
              <p className="text-xl font-semibold text-white">${totalValue.toFixed(4)}</p>
            </div>
            <div className="bg-[#111] border border-[#1a1a1a] rounded-lg p-4">
              <p className="text-[#555] text-xs mb-1">Total PnL</p>
              <p className={`text-xl font-semibold ${totalPnL >= 0 ? "text-[#00d26a]" : "text-[#ff4757]"}`}>
                {totalPnL >= 0 ? "+" : ""}{totalPnL.toFixed(4)}
              </p>
            </div>
            <div className="bg-[#111] border border-[#1a1a1a] rounded-lg p-4">
              <p className="text-[#555] text-xs mb-1">Positions</p>
              <p className="text-xl font-semibold text-white">{positions.length}</p>
            </div>
          </div>

          {/* Tabs */}
          <div className="flex gap-1 mb-6 bg-[#111] p-1 rounded-lg border border-[#1a1a1a] w-fit">
            {[
              ...
              <button
                ...
                className={`flex items-center gap-2 px-4 py-2 rounded text-sm transition-colors ${
                  activeTab === tab.id ? "bg-[#1a1a1a] text-white" : "text-[#555] hover:text-white"
                }`}
              >
                <tab.icon className="w-4 h-4" />
                {tab.label}
                <span className="text-[#444]">({tab.count})</span>
              </button>
            ))}
          </div>
          ...
          {isLoading ? (
            ...
            <>
              {activeTab === "holdings" && (
                holdings.length === 0 ? (
                  <div className="bg-[#111] border border-[#1a1a1a] rounded-lg p-10 text-center">
                    <Wallet className="w-10 h-10 text-[#333] mx-auto mb-3" />
                    <p className="text-white font-medium mb-1">No Holdings</p>
                    <p className="text-[#555] text-sm mb-4">Start trading to build your portfolio</p>
                    <Link href="/" className="text-[#00d26a] text-sm hover:underline">Explore Tokens</Link>
                  </div>
                ) : (
              ...
              {activeTab === "positions" && (
                positions.length === 0 ? (
                  <div className="bg-[#111] border border-[#1a1a1a] rounded-lg p-10 text-center">
                    ...
                    <p className="text-[#555] text-sm mb-4">Open a perpetual position to get started</p>
                    <Link href="/" className="text-[#00d26a] text-sm hover:underline">Trade Now</Link>
                  </div>
                ) : (
              ...
              {activeTab === "copy" && (
                copyPositions.length === 0 ? (
                  <div className="bg-[#111] border border-[#1a1a1a] rounded-lg p-10 text-center">
                    ...
                    <p className="text-[#555] text-sm mb-4">Subscribe to a leader to start copy trading</p>
                    <Link href="/copy-trading" className="text-[#00d26a] text-sm hover:underline">Find Leaders</Link>
                  </div>
                ) : (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="bg-[#111] border border-[#1a1a1a] rounded-lg">
                  {transactions.length === 0 ? (
                    <div className="p-10 text-center">
                      <History className="w-10 h-10 text-[#333] mx-auto mb-3" />
                      <p className="text-white font-medium mb-1">No Transactions</p>
                      <p className="text-[#555] text-sm">Your history will appear here</p>
                    </div>
                  ) : (
                    <div className="p-4">
