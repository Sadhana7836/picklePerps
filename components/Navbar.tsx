...
import { Search, Flame, Star, Settings, ChevronDown, Wallet, X, Menu } from "lucide-react";
import Link from "next/link";
import Image from "next/image";
import { usePathname } from "next/navigation";
import { useAppActions, useSearchQuery } from "@/lib/store";
...
function WalletButton() {
  ...
  if (!walletConnected) {
    return (
      <button
        ...
        className="flex items-center gap-1.5 bg-[#1a1a1a] hover:bg-[#252525] border border-[#2a2a2a] hover:border-[#00d26a]/50 text-white text-xs px-3 py-1.5 rounded-lg transition-all"
      >
  ...
  return (
    <button
      onClick={disconnect}
      className="flex items-center gap-2 bg-[#1a1a1a] hover:bg-[#252525] border border-[#2a2a2a] text-xs px-2.5 py-1.5 rounded-lg transition-all group"
    >
      <div className="w-5 h-5 rounded-full bg-[#00d26a] flex items-center justify-center">
      ...
      <span className="text-[#00d26a]">{walletBalance} XLM</span>
    </button>
  );
}
...
export const Navbar = memo(function Navbar() {
  ...
  return (
    <nav className="flex items-center justify-between px-2 sm:px-4 py-2 bg-[#0d0d0d] border-b border-[#1a1a1a] h-14">
      {/* Left Section - Logo and Nav Links */}
      <div className="flex items-center gap-2 sm:gap-4 lg:gap-6">
        {/* Hamburger Menu - visible on smaller screens */}
        <button
          onClick={toggleSidebar}
          className="p-1.5 rounded-lg hover:bg-[#1a1a1a] xl:hidden"
        ...
        <Link href="/" onClick={() => setSelectedToken(null)} className="flex items-center gap-2 hover:opacity-80 transition-opacity group">
          <Image
            ...
            width={36}
            height={36}
            className="object-contain transition-transform group-hover:scale-105 pt-1"
            priority
          />
          <span className="text-white text-base lg:text-lg font-bold hidden sm:inline">StellaPerps</span>
        </Link>
          ...
          {navLinks.map((link) => {
            ...
            return (
              <Link
                key={link.name}
                href={link.href}
                className={`text-xs xl:text-sm hover:text-[#00d26a] transition-colors whitespace-nowrap ${
                  isActive ? "text-[#00d26a]" : "text-[#888888]"
                }`}
              >
                {link.name}
              </Link>
      ...
      <div className="flex items-center gap-1.5 sm:gap-3">
        ...
        <div className={`flex items-center bg-[#1a1a1a] rounded-full px-2 sm:px-3 py-1.5 gap-2 w-[120px] sm:w-[180px] lg:w-[240px] border transition-colors ${searchQuery ? 'border-[#00d26a]/50' : 'border-transparent'}`}>
          ...
          <input
            ...
            placeholder="Search..."
            className="bg-transparent text-xs sm:text-sm text-white placeholder-[#888888] outline-none w-full min-w-0"
          />
          {searchQuery ? (
        ...
        <div className="hidden md:flex items-center gap-1 text-[#888888] hover:text-white cursor-pointer">
        ...
        <button
          ...
          className="hidden sm:flex items-center gap-1 bg-[#1a1a1a] rounded px-2 py-1 text-xs text-[#888888] hover:text-white hover:bg-[#252525] transition-colors"
        >
        ...
        <Star className="w-4 h-4 text-[#888888] hover:text-yellow-400 cursor-pointer hidden sm:block" />
        <Settings className="w-4 h-4 text-[#888888] hover:text-white cursor-pointer hidden sm:block" />
      ...
      {showAppModal && (
        ...
        >
          <div
            className="relative bg-[#1a1a1a] border border-[#2a2a2a] rounded-xl p-4 max-w-[280px] w-full mx-4 shadow-2xl"
            onClick={(e) => e.stopPropagation()}
          >
            {/* Close button */}
            <button
              onClick={() => setShowAppModal(false)}
              className="absolute top-2 right-2 p-1 rounded-lg hover:bg-[#2a2a2a] text-[#888] hover:text-white transition-colors"
            >
              <X className="w-4 h-4" />
            </button>
            ...
            <div className="flex flex-col items-center text-center">
              ...
              {/* Badge */}
              <div className="inline-flex items-center gap-1.5 bg-[#00d26a]/10 text-[#00d26a] text-xs px-3 py-1.5 rounded-full">
